...
class PgQuery extends Query {
    ...
    join (table, tk, op, wk,type = 'INNER') {
        this._joinPlaceholder += ` ${type} JOIN ${table} ON ${tk} ${op} ${wk} `;
        return this;
    }

    leftJoin(table, tk, op, wk) {
        return this.join(table, tk, op, wk, 'LEFT');
    }

    rightJoin(table, tk, op, wk) {
        return this.join(table, tk, op, wk, 'RIGHT');
    }
...
function buildWhere(key, op, value, or, whereIndex) {
    ...
    if (op === 'between' && Array.isArray(value))
        return [{
            value: `${key} ${op} $${++whereIndex} AND $${++whereIndex}`,
            type
        }, [value[0], value[1]]];
