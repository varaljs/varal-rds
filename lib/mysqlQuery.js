...
class MysqlQuery {
    ...
    orderBy(key, type = 'ASC') {
        this._orderPlaceholder.push(`${key} ${type.toLocaleUpperCase()}`);
        return this;
    }
    ...
    join(table, type = 'INNER') {
        this._joinPlaceholder = `${type} JOIN ${table}`;
        return this;
    }
    ...
    on(key1, key2) {
        this._joinOnPlaceholder.push(`${key1} = ${key2}`);
        return this;
    }
    ...
    buildQuery() {
        ...
        if (this._method === 'SELECT') {
            ...
            if (this._joinPlaceholder && this._joinOnPlaceholder.length > 0)
                query += ` ${this._joinPlaceholder} ON ${this._joinOnPlaceholder.join(' AND ')}`;
        }
