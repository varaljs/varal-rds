...
class Model extends CollectionObj {
    ...
    static getClassName() {
        return /class\s(\S*)\s/.exec(this.toString())[1];
    }
    ...
    static async all() {
        return await this.getServer().table(this.getTable(), this).get();
    }

    static async find(value) {
        return await this.getServer().table(this.getTable(), this).where(this.getPK(), value).first();
    }

    static async create(data) {
        return await this.getServer().table(this.getTable(), this).insert(data);
    }
